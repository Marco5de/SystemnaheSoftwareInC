...
int main(int argc, char *argv[]){
	...
	//creating list beforehand, should be done later to prevent case that there are less files than elements
	du_hog_readdir(path);
	debug_printList();	
...
//helper functions
void compareSize(off_t size, char* path, char *fname){
	//smallest Element of List is always the first Ele
	if(size < flist->fileEle->size){
	...
	int counter = 0;
	struct fileEle *ele = flist;
	while(size > ele->next->fileEle->size){
		if(!ele->next)
			ele = ele->next;
	}
	if(!counter){
	...
	//einfügen des element in die Liste 
	#ifdef DEBUG
	printf("---Inserting new Element into list ---\n");
	#endif

	ele->next->fileEle->size = size;
	ele->next->fileEle->path = assemblePathstring(path,fname);
